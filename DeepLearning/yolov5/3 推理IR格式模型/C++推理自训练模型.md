...
```c++
/*
    c++ 语言推理官方yolov5s_6.2 IR格式模型
    https://www.jianshu.com/p/2f822ae0e582
    win10 vs2019 yolov5_6.2 opencv4.7.0
...
std::string model_file = "E:\\Projects\\Detect_Segment_Classify\\yolov5-6.2\\weights\\YOLOv5s_gesture\\weights\\best_openvino_model\\best.xml";
//std::string model_file = "E:\\Projects\\Detect_Segment_Classify\\yolov5-6.2\\weights\\YOLOv5s_prune_soccer\\best_openvino_model\\best.xml";
//std::string image_file = "C:\\Users\\Simple\\Desktop\\test\\g_pic.jpg";
//std::string image_file = "C:\\Users\\Simple\\Desktop\\test\\scroll.jpg";
std::vector<cv::Scalar> colors = { cv::Scalar(0, 0, 255) , cv::Scalar(0, 255, 0) , cv::Scalar(255, 0, 0) ,
                                   cv::Scalar(255, 255, 0) , cv::Scalar(0, 255, 255) , cv::Scalar(255, 0, 255) };
const std::vector<std::string> class_names = { "2", "4", "1", "ok", "fist", "5"};
//const std::vector<std::string> class_names = { "soccer" };
...
int main(int argc, char* argv[]) {
    ...
    auto compiled_model = core.compile_model(model_file, "CPU"); // CPU infer
    ...
    while (true)
    {
        ...
        for (int i = 0; i < output_buffer.rows; i++) {
            ...
            cv::Mat classes_scores = output_buffer.row(i).colRange(5, 11); // 不知道为啥写5，10，反正不报错
            //cv::Mat classes_scores = output_buffer.row(i).colRange(1, 6); // 不知道为啥写5，10，反正不报错
